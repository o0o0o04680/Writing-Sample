...
print(vif_df) ##No-autocorrelation
...
def hausman_test(res_A, res_B, labelA = "A", labelB = "B"):
    ...
    print("Hausman Test")
    print(f"data:  {name_A} and {name_B}")
...
pdata = add_lags(pdata, lag_base, l=1)
...
for col in ['salary','firm_size','leverage','rd_intensity','cash_ratio','ceo_duality']:
    pdata[f"lag_{col}"] = pdata.groupby(level=0)[col].shift(1)
